...
- 예약 시각: 매일 09:37~18:37 KST, 매시간 10회 (`37 9-18 * * *`, `Asia/Seoul`)
- 최초 배포 성공 실행: <https://github.com/ggs2535/hongseong-auction-report/actions/runs/29503452625>
- 폐기된 즉시조회 기능 기록: <https://github.com/ggs2535/hongseong-auction-report/pull/1>
...
2026-07-16의 과거 즉시조회 검증에서는 소유자 gate, 61개 테스트, durable 예약,
live 수집, 결과 저장, Pages 배포, 이슈 댓글과 자동 종료가 모두 성공했습니다.
두 검증 실행 모두 법원코드는 정상 확인했으나 첫 목록 요청 전에
...
반복하지 말고 법원 서비스가 회복된 뒤 다음 예약 실행이 시도하도록 둡니다. 같은 오류가
...
2026-07-18부터 공개 페이지와 수집 workflow의 이슈 기반 즉시조회 및 수동 실행
트리거를 제거했습니다. `src/query-safety-policy.js`의 최소 안전 간격과 당일
`BLOCKED` 후속 실행 금지를 제거하지 마세요. `data/query-safety-state.json`은 실제
호출 전에 원격 저장소에 먼저 커밋되는 durable 안전 상태입니다. 손상되거나 누락되면
예약 조회를 허용하지 않는 fail-closed 동작을 유지해야 합니다.
