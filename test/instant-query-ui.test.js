...
test("모바일 보고서는 토큰 없이 소유자 전용 즉시 조회 요청 링크를 제공한다", () => {
  ...
  assert.match(html, /app\.js\?v=4/u);
  assert.match(html, /style\.css\?v=4/u);
  assert.doesNotMatch(
    html,
...
test("서비스 워커는 명시적 새로고침에 오프라인 cache를 최신처럼 반환하지 않는다", () => {
  ...
  assert.match(worker, /static-v4/u);
  assert.match(worker, /\.\/app\.js\?v=4/u);
  assert.match(worker, /\.\/style\.css\?v=4/u);
