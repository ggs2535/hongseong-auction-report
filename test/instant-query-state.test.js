...
const {
  ...
  writeOutcomeOutputs,
} = require("../src/instant-query-state");
...
test("즉시 조회 시작 전에 durable 예약 상태를 만든다", () => {
  ...
  assert.equal(validateInstantQueryState(reserved), true);
});
...
test("손상된 durable 상태는 수정하지 않고 거부한다", () => {
  assert.equal(validateInstantQueryState({}), false);
  assert.throws(
    () =>
      reserveState(
        { schemaVersion: 1 },
...
test("완전성 outcome을 GitHub job output으로 기록한다", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "instant-outcome-"));
