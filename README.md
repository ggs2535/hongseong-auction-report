...
보고서는 매일 한국시간 `09:37`부터 `18:37`까지 한 시간 간격으로 자동 갱신됩니다.
공개 화면에서 GitHub 이슈를 만들거나 workflow를 수동 실행하는 즉시조회 기능은
제공하지 않습니다. 휴대폰에서는 **최신 결과 확인**을 눌러 새로 게시된 보고서가
있는지만 확인하면 됩니다.

당일 `BLOCKED`가 기록되거나 법원 요청 중 실행이 비정상 종료되면 그날 남은 예약
조회를 모두 건너뜁니다. 실제 법원 호출 직전 시각과 결과는
`data/query-safety-state.json`에 먼저 기록하므로 workflow 실패 후에도 보호가
유지됩니다.
...
- 예약 조회도 최소 10분 안전 간격과 당일 `BLOCKED` 후속 실행 금지를 적용합니다.
...
코드를 기본 브랜치에 올린 뒤 다음 자동조회 시간에 **Actions → Update auction report**를
확인합니다. 순서는 의존성 설치, 테스트, Chromium 설치, live 조회, 결과 커밋,
Pages 배포입니다.
...
UI 검색 결과가 기본 10건 한 페이지를 넘으면 공식 `페이지당 40` 옵션으로 바꾼
응답을 새 1페이지로 사용합니다. 크기 변경 요청도 목록 호출 상한과 3~5초 간격에
포함하며, 뒤 페이지가 계속 40건 계약을 유지하는지 검증합니다.
...
`.github/workflows/update-auction.yml`은 `Asia/Seoul` 기준 매일 `09:37`, `10:37`,
`11:37`, `12:37`, `13:37`, `14:37`, `15:37`, `16:37`, `17:37`, `18:37`에
실행됩니다. GitHub의 정각 혼잡을 피하도록 37분에 맞췄고, 중복 실행은 concurrency
설정으로 막습니다. 19:00부터 다음 날 09:00 전까지는 실행하지 않습니다.

한 실행에서 `BLOCKED`가 확인되거나 시작한 조회가 실패·중단되면 안전 상태가 그날
남은 예약 실행을 막습니다. 다음 한국시간 날짜부터 다시 자동 시도합니다.
...
| `data/query-safety-state.json` | 차단·비정상 종료 뒤 후속 예약을 막는 durable 안전 상태 |
| `public/index.html` | 오늘 상태와 표시용 last-good을 내장한 Pages 문서 |

같은 날 여러 예약 실행이 완료되면 그 날짜의 history 파일은 마지막 실행으로 교체됩니다.
...
| `COURT_CODE_FALLBACK` | `B000281` | 법원 목록 네트워크 오류 때만 쓰며 공식 UI 요청으로 재검증 |
