...
- 예약 시각: `Asia/Seoul` 기준 매일 09:37~18:37, 매시간 10회
...
같은 날 과거 즉시조회 기능을 PR #1로 배포한 뒤 이슈 #2와 #4로 실동작을 검증했습니다.
...
11. 운영 workflow에 `schedule` 외 법원 조회 트리거가 없는지 확인합니다.
...
14. 운영 수집 workflow는 예약 실행만 허용하며 이슈·수동 조회 트리거를 추가하지 않습니다.
15. 예약 조회의 10분 안전 간격을 줄이거나 당일 `BLOCKED` 후속 실행 금지를 우회하지 않습니다.
16. GitHub 토큰을 HTML, JavaScript, service worker, localStorage에 저장하지 않습니다.
17. `data/query-safety-state.json`은 실제 호출 전에 커밋하며, 손상 시 fail-closed합니다.
18. 예약 수집은 고정 concurrency 그룹으로 직렬화하고 진행 중 실행을 취소하지 않습니다.
...
| `src/query-safety-policy.js` | 예약 조회 안전 간격과 당일 BLOCKED 후속 실행 금지 |
| `src/query-safety-state.js` | 실제 호출 전 durable 예약과 complete/blocked/incomplete 결과 기록 |
| `data/query-safety-state.json` | workflow 실패·취소 후에도 남는 조회 안전 상태 |
...
9. 예약 조회가 시작되지 않으면 workflow cron, `Asia/Seoul` timezone, 기본 브랜치와 60일 비활동 여부를 확인합니다.
10. gate의 Actions Summary에서 당일 차단·비정상 종료·안전 간격 거부 사유를 확인합니다.
...
- 운영 workflow가 09:37~18:37 KST 매시간 실행되고 이슈·수동 트리거가 없음
- 예약 조회 10분 안전 간격과 당일 BLOCKED 후속 실행 금지
- 실제 호출 전 durable 예약 커밋과 비정상 종료 후 당일 재조회 금지
- complete/blocked/incomplete 결과가 안전 상태에 구분되어 기록됨
